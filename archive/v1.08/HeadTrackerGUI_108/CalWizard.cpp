...
System::Void CalWizard::Initialize() {
    CurrentStep = 0;
...
System::Void CalWizard::MainUpdateTimer_Tick(System::Object^ sender, System::EventArgs^ e) {
    if (TrackerWrap::Tracker->IsOpen()) {
        while (TrackerWrap::Tracker->IsDataAvailable()) {
...
System::Void CalWizard::UpdateGUI() {
    ...
    if (CalMode == MINMAXTRACKING && TrackMinMax) UpdateCalc();
}
//--------------------------------------------------------------------------------------
// Func: UpdateCalc
...
{
    if ((WizardPictureBox == nullptr) || (WizardInstructionsLbl == nullptr)) return;
    System::Resources::ResourceManager^ rm = gcnew ResourceManager("HeadTrackerGUI.Resources", GetType()->Assembly);
    switch ( Step )
    {
    case 0:
        {
