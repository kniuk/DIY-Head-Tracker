...
#else
  #define HMC_ADDR 0x0D        // The address of QMC5883
  #define HMC_X_ADDR (0x00)    // Start address for x-axis. 
#endif
...
#define MAG0MAX 625           // Not needed as we do the calibration via GUI (NEB)
#define MAG1MAX 625
#define MAG2MAX 625
#define MAG0MIN -625
#define MAG1MIN -625
...
float rollAngle = 90;       // Roll angle  // Was 0 (NEB)
...
#ifdef HMC5883
       magRaw[0] = (int)sensorBuffer[1] | ((int)sensorBuffer[0] << 8);  // 1°=MSB 2°=LSB
       magRaw[1] = (int)sensorBuffer[3] | ((int)sensorBuffer[2] << 8);
       magRaw[2] = (int)sensorBuffer[5] | ((int)sensorBuffer[4] << 8);
#else // QMC5883, Y and Z are inverted compared to HMC5883 (NEB)
       magRaw[0] = (int)sensorBuffer[0] | ((int)sensorBuffer[1] << 8);  // 1°=LSB 2°=MSB.
       magRaw[2] = (int)sensorBuffer[2] | ((int)sensorBuffer[3] << 8);
       magRaw[1] = (int)sensorBuffer[4] | ((int)sensorBuffer[5] << 8);
#endif
}
...
{
#ifdef HMC5883 // NEB
    WriteToI2C(HMC_ADDR, 0x00, 0b00010001); // 15 Hz + Positive bias
    ...
    WriteToI2C(HMC_ADDR, 0x00, 0b00010010); // 15 Hz + Negative bias
    ...
    WriteToI2C(HMC_ADDR, 0x00, 0b00010000); // 15 Hz + Normal function
...
#else // QMC5883
    magGain[0] = 1;// Do not exists neg/pos offset calculation in QMC5883, so we assume +1. On HMC the gain is near to 1 (NEB) 
    magGain[1] = 1;
    magGain[2] = 1;
#endif
...
#if (DEBUG)
    Serial.print("Mag neg offset:");
    Serial.print(magNegOff[0]);
    Serial.print(",");   
    Serial.print(magNegOff[1]);
    Serial.print(",");      
    Serial.println(magNegOff[2]);

    Serial.print("Mag pos offset:");
...
void MagCalc()
{
    ...
    // Set gain:
#ifdef HMC5883 // Use magGain only for HMC, with QMC is always +1 (NEB)
    magRaw[0] *= magGain[0];
    magRaw[1] *= magGain[1];
    magRaw[2] *= magGain[2];
#endif
...
#ifdef HMC5883  (NEB)
    ReadFromI2C(HMC_ADDR, 0x1A, 1);       // HMC ID (72)
#else
    ReadFromI2C(HMC_ADDR, 0x0D, 1);       // QMC ID (255)
#endif   
...
#ifdef HMC5883
    // HMC5883
    WriteToI2C(HMC_ADDR, 0x02, 0x00);     // Run in continuous mode
#else
    // QMC5883
    WriteToI2C(HMC_ADDR, 0x09, 0xC5);     // register 9 -> 0xC5=1100 0101= Continuous, 50Hz, 2Gauss, 64 oversamples (Higher sensitivity) (NEB)
//     WriteToI2C(HMC_ADDR, 0x09, 0xD5);  // register 9 -> 0xD5=1101 0101= Continuous, 50Hz, 8Gauss, 64 oversamples (Lower sensitivity) (NEB)
#endif
...
{
#ifdef HMC5883  // These data has not sense with mag QMC5883 (NEB)
   Serial.print("Mag cal:");
   Serial.print(magNegOff[0] - magPosOff[0]);
   Serial.print(",");   
   Serial.print(magNegOff[1] - magPosOff[1]);
