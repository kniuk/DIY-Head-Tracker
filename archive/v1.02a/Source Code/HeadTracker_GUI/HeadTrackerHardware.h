...
public: // methods
    ...
    System::Void Open(String^ ComPort)
    {
        ...
        // Sometimes the early version retrieval doesn't work. Try again.
        if ( _FWVersion == 0.0 )
            GetVersion();

        // If it failed again, the open request failed.
        if ( _FWVersion == 0.0 )
            this->Close();
    }
        ...
        if (Port->IsOpen)
        {
            ...
            try
            {
                if ( Line->Substring(0, 5) == "$SET$" )
                {
                    ...
                    {
                        ht.LPTiltRoll = (int)Convert::ToDouble(items[0]);
                        ht.LPPan = (int)Convert::ToDouble(items[1]);                 
                        ht.GyroWeightTiltRoll = (int)Convert::ToDouble(items[2]);        
                        ht.GyroWeightPan = (int)Convert::ToDouble(items[3]);         
                        ht.ServoGainTilt = (int)Convert::ToDouble(items[4]);          
                        ht.ServoGainPan = (int)Convert::ToDouble(items[5]);         
                        ht.ServoGainRoll = (int)Convert::ToDouble(items[6]);         
    ...
    float GetVersion()
    {
        if ( Port->IsOpen)
        {
            ...
            try
            {
                if ( versLine->Length > 12 && versLine->Substring(0, 12) == "FW Version: " )
                {
                    String^ vers = versLine->Substring(12);
                    // Parse the rest of the response
                    array<wchar_t>^ delim = {',','\r','\n'};
                    array<String^>^ items = vers->Split(delim);
                    _FWVersion = Convert::ToSingle(items[0]);
                }
                else
                {
                    _FWVersion = 0.0;
